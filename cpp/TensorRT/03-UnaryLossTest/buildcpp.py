"""
    example:
    $ python buildcpp.py -d f -i f -o h -x kLOG

    -d data type:   {f: float, i: unsigned int, b: bool}
...
#define OP UnaryOperation::$OP$

std::string log_file_name = "$LOG_FILENAME$";
#define IN_TYPE $IN_TYPE$
#define IN_VALUE_TYPE $IN_VALUE_TYPE$
...
assert inType in typeSet
assert outType in typeSet

code_IN_TYPE = typeSet_TRT[typeSet.index(inType)]
code_IN_VALUE_TYPE = typeSet_FN[typeSet.index(inType)]

code_OUT_TYPE = typeSet_TRT[typeSet.index(outType)]
code_OUT_VALUE_TYPE = typeSet_FN[typeSet.index(outType)]

code_OP = op
code_LOG_FILENAME = f"./logs/{inType}_{outType}_{op}.txt"

code_INIT_INPUT_DATA = ""
if data == "f":
    code_INIT_INPUT_DATA = "std::vector<IN_VALUE_TYPE> data = {0.0f, 0.5f, 1.0f, 1.5f, 2.0f};"
if data == "i":
    code_INIT_INPUT_DATA = "std::vector<IN_VALUE_TYPE> data = { 1, 2, 3, 4, 5};"
if data == "b":
    code_INIT_INPUT_DATA = "std::vector<IN_VALUE_TYPE> data = {true, false, true, false, true};"

TGT_CPP_TXT = RAW_CPP_TXT\
    .replace("$LOG_FILENAME$", code_LOG_FILENAME)\
    .replace("$IN_TYPE$", code_IN_TYPE)\
    .replace("$IN_VALUE_TYPE$", code_IN_VALUE_TYPE)\
    .replace("$OUT_TYPE$", code_OUT_TYPE)\
    .replace("$OUT_VALUE_TYPE$", code_OUT_VALUE_TYPE)\
    .replace("$OP$", code_OP)\
    .replace("$INIT_INPUT_DATA$", code_INIT_INPUT_DATA)

with open("./losstest.cpp", "w") as f:
    f.write(TGT_CPP_TXT)
