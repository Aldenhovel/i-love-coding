# Construct Concat Network

## 修改网络

### 修改输入数据float

1. 首先需要在 `params` 中重新规划输入是两个向量：
...
2. 在 `processInput` 构造两个输入数据，即两个 `std::vector<float>` ：
...
3. 这两个输入数据对应两个 `hostDataBuffer` 也需要创建并传入数值：
...
1. 原来只预览一个 `Input` 现在改为将创造的两个 `vector<float>` ： `fileDataOnes` `fileDataTwos` 都输出来：
...
1. 原程序建造了一个 `ITensor` 指针根据 `params` 指向传入的向量，可以看到他是根据输入名字指定的。
   ...
   在前面已经 `inputTensorNames` 中压入了所需的两个向量名，这里直接为他们创建两个 `ITensor` ：
...
2. 检查文档发现 Concat 操作需要的输入是 `ITensor*` 而不是一般层的 `ITensor` ，因此需要将 `tensor1` 和 `tensor2` 用一个 `ITensor*` 表示出来：

   ```cpp
   ITensor* tensors[] = {tensor1, tensor2};
   ```

3. 为网络新增 `IConcatenationLayer` 并将上面的 `tensors` 作为输入，参数 `2` 指参与拼接的有 2 个向量，`concat_layer->setAxis(2)` 指定了这个 non-batch 向量 `[C, H, W]` 中的宽方向。
...
1. 返回的数据是 `float*` 指针，因为将两个矩阵 concat 了所所以输出上也要输出 2 倍元素，在前面中由于对 `W` 方向做了 Concatenation 所以应该输出每行 2 倍元素：
   ...
   bool SampleMNISTAPI::verifyOutput(const samplesCommon::BufferManager& buffers)
   {
       ...
       {
           // 每行宽度变成原来 2 倍
           std::cout << prob[i] << (((i + 1) % mParams.inputW * 2) ? "" : "\n");
       }
   }
   ```
