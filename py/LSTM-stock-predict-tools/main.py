...
def vaild_step(model, loader, epoch, device):
    ...
    for i, (seqs, seqlens) in pbar:
        ...
        logits, sorted_seqs, sorted_decode_len, sorted_seq_indices = model(seqs, seqlens)
        # logits = pack_padded_sequence(logits, sorted_decode_len).data
        # gt = pack_padded_sequence(sorted_seqs, sorted_decode_len, batch_first=True).data
...
def inference(seq, seqlen, model, beam_size, device):
    ...
    model.to(device)
    with torch.no_grad():
        h_state, c_state = model.init_h0(torch.ones(1, model.vocab_size).to(device)), model.init_c0(
            torch.ones(1, model.vocab_size).to(device))
        ...
        while True:
            ...
            incomplete_indices = [indice for indice, next_token in enumerate(next_tokens) if next_token != 102]
...
def plot_res(front, res, gt=None, gtlen=None):
    ...
    for i in range(len(res)):
        plt.plot((front + res[i])[:xmax], label="pred", color="green")
    plt.plot(front, color="black")
    plt.vlines(len(front) - 1, 50, front[-1], linestyles="dashed", colors="blue")
    plt.hlines(51, 0, xmax, linestyles="dashed", colors="red")
    # plt.xlim([0, xmax])
    # plt.ylim([0, 100])
    plt.legend()
    plt.show()
...
import matplotlib.pyplot as plt
data = train_ds.reader.readyaml("../vaild/2023-01-18-003007.yaml")
seq = data["stdfenshi"]
...
for i in range(1, seqlen // 10):
    ...
    mskseqlen = msk
    res = inference(mskseq, mskseqlen, model, beam_size=10, device=device)
    plot_res(front=mskseq[:mskseqlen].detach().cpu().numpy().tolist(), res=res, gt=seq, gtlen=seqlen)